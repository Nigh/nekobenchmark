...
压缩包中的 `MapleMono-Regular.ttf` 必须与可执行文件放在同一目录。

- `Z`、`X`、`Space`、方向键或鼠标左键：开始测试，以及在目标颜色出现后记录反应。
- 长按任一有效键至少 1 秒后松开：松开时也会作为一次输入触发。
- `C`：切换红→绿与黄→蓝配色。
- `L`：切换英文与中文窗口状态文本。
- `Esc`：退出。
...
界面使用 Maple Mono 常规字重，结果页突出显示中位数，并将平均值、标准差和显示刷新率误差分栏展示。字体在首次使用时缓存为纹理，因此不会在每个渲染帧重复栅格化。

Maple Mono 以 [SIL Open Font License 1.1](assets/MapleMono-OFL.txt) 随发布包分发。
